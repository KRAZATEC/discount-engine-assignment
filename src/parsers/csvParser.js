 ...
 *
 * This is one of three "input adapters" (alongside nlParser.js and
 * pdfParser.js) that all produce the same two shapes the engine consumes:
 * DiscountRule[] and CartItem[]. The engine never imports this file.
 *
 * Expected rules.csv columns:
 *   rule_id, scope, applies_to, type, value, stackable, min_cart_value
 *
 *   - applies_to is required for scope "brand" / "platform", and ignored
 *     (may be blank) for scope "cart".
 *   - min_cart_value is required for scope "cart", and ignored for the
 *     other two scopes.
 *
...
export function parseRulesCSV(csvText) {
  ...
  rows.forEach((row, i) => {
    ...
    if (type === 'percentage' && value > 100) {
      errors.push(`Row ${rowNum}: percentage value cannot exceed 100, got "${row.value}"`)
      return
    }

    const stackableStr = String(row.stackable).trim().toLowerCase()
    const stackable = stackableStr === 'true' || stackableStr === '1' || stackableStr === 'yes'

    // applies_to is required for brand/platform scope, irrelevant for cart scope
    if (scope !== 'cart' && !row.applies_to) {
      errors.push(`Row ${rowNum}: applies_to is required for scope "${scope}"`)
      return
    }

    // min_cart_value is required for cart scope, irrelevant otherwise
    let minCartValue = null
    if (scope === 'cart') {
      if (row.min_cart_value === undefined || row.min_cart_value === '') {
        errors.push(`Row ${rowNum}: min_cart_value is required for scope "cart"`)
        return
      }
